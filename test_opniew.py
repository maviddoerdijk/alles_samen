...
plt.plot([i for i in range(window_size - 1, window_size + future_steps_size)],np.append([avg_history[-1]],avg_future), label="Future")
plt.plot([i for i in range(window_size - 1, window_size + future_steps_size)],np.append([avg_history[-1]],avg_predicted), label="Predicted")
plt.legend()
plt.show()
